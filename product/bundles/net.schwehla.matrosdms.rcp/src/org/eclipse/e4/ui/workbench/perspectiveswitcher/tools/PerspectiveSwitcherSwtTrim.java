...
public class PerspectiveSwitcherSwtTrim implements IPerspectiveSwitcherControl {
	...
	void createControl(Composite parent, MToolControl control) {
		...
		List<MPerspectiveStack> appPerspectiveStacks = E4Util
				.getMatchingChildren(app, MPerspectiveStack.class);
		
		List<MPerspectiveStack> perspectives = modelService.findElements(app, null, MPerspectiveStack.class, null);
	 		
		
		if (perspectives.size() > 0) {
			for (MPerspectiveStack stack : perspectives)
