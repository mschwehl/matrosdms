...
public abstract class MatrosTooltip {
...
private MatrosTooltipOwnerControlListener listener;

private HashMap<String, Object> data;

// Ensure that only one tooltip is active in time
protected  Shell CURRENT_SHELL;
 ...
 */
public MatrosTooltip(Control control) {
 ...
 * @see #NO_RECREATE
 */
public MatrosTooltip(Control control, int style, boolean manualActivation) {
	this.control = control;
	this.style = style;
	this.listener = new MatrosTooltipOwnerControlListener();
	this.shellListener = event -> {
		if (MatrosTooltip.this.control != null
				&& !MatrosTooltip.this.control.isDisposed()) {
			MatrosTooltip.this.control.getDisplay().asyncExec(() -> {
				// Check if the new active shell is the tooltip
				// itself
				if (MatrosTooltip.this.control != null && !MatrosTooltip.this.control.isDisposed()
						&& MatrosTooltip.this.control.getDisplay().getActiveShell() != CURRENT_SHELL) {
					toolTipHide(CURRENT_SHELL, event);
				}
			});
		}
	};
 ...
 */
protected boolean shouldCreateMatrosTooltip(Event event) {
	if ((style & NO_RECREATE) != 0) {
		Object tmp = getMatrosTooltipArea(event);
 ...
 */
private boolean shouldHideMatrosTooltip(Event event) {
 ...
 *         {@link ViewerCell})
 */
protected Object getMatrosTooltipArea(Event event) {
	return control;
}
...
private Shell toolTipCreate(final Event event) {
	if (shouldCreateMatrosTooltip(event)) {
...
private void toolTipShow(Shell tip, Event event) {
	if (!tip.isDisposed()) {
		currentArea = getMatrosTooltipArea(event);
		createMatrosTooltipContentArea(event, tip);
...
private void toolTipHide(Shell tip, Event event) {
	if (tip != null && !tip.isDisposed() && shouldHideMatrosTooltip(event)) {
		control.getShell().removeListener(SWT.Deactivate, shellListener);
		currentArea = null;
		passOnEvent(tip, event);
		tip.dispose();
		CURRENT_SHELL = null;
		afterHideMatrosTooltip(event);
	}
...
private void toolTipOpen(final Shell shell, final Event event) {
	...
	if (CURRENT_SHELL != null) {
		toolTipHide(CURRENT_SHELL, null);
	}
	...
	if (popupDelay > 0) {
		...
		toolTipShow(CURRENT_SHELL, event);
	}
 ...
 */
protected abstract Composite createMatrosTooltipContentArea(Event event,
		Composite parent);
 ...
 */
protected void afterHideMatrosTooltip(Event event) {
	...
	if (CURRENT_SHELL != null && !CURRENT_SHELL.isDisposed()) {
		...
		if (hideOnMouseDown != this.hideOnMouseDown) {
			control.getDisplay().syncExec(() -> {
				if (CURRENT_SHELL != null
						&& CURRENT_SHELL.isDisposed()) {
					toolTipHookByTypeRecursively(CURRENT_SHELL,
...
private class MatrosTooltipOwnerControlListener implements Listener {
	@Override
	public void handleEvent(Event event) {
		...
		case SWT.Resize:
			...
			Point buttonPoint = control.toDisplay(control.getLocation());
			
			CURRENT_SHELL.setLocation(buttonPoint);
		...
		case SWT.MouseExit:
			 ...
			 */
			if (CURRENT_SHELL != null && !CURRENT_SHELL.isDisposed()) {
				if (CURRENT_SHELL.getBounds().contains(
