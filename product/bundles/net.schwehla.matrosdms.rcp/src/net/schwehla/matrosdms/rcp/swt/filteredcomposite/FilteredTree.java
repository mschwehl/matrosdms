...
public class FilteredTree extends Composite {
	...
	private Button smartFilterCheckbox;
	...
	TagSmartTreeFilter _smartTagFilter;

	public FilteredTree(Composite parent, int treeStyle, PatternFilter filter, TagSmartTreeFilter smartTagFilter) {
		super(parent, SWT.NONE);
		init(treeStyle, filter,smartTagFilter);
	}
	 ...
	 * @since 3.3
	 */
	protected void init(int treeStyle, PatternFilter filter, TagSmartTreeFilter smartTagFilter) {
		patternFilter = filter;
		_smartTagFilter = smartTagFilter;
		...
		if (smartTagFilter != null) {
			
			addSmartFilterListener( new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
								   ...
								   Button button = (Button) e.widget;
							        if (button.getSelection()) {
							        	smartTagFilter.setLocalContextLookup(true);
							        } else {
							          	smartTagFilter.setLocalContextLookup(false);
							        }
	...
	protected Composite createFilterControls(Composite parent) {
		...
		createSmartSearch(parent);
		return parent;
	}

	

	private void createSmartSearch(Composite parent) {
		
		if (_smartTagFilter != null) {
			smartFilterCheckbox = new Button(parent, SWT.CHECK);
		}
	...
	protected Control createTreeControl(Composite parent, int style) {
		...
		if (_smartTagFilter != null) {
			treeViewer.addFilter(_smartTagFilter);
		}
		...
		filterText.getAccessible().addAccessibleListener(new AccessibleAdapter() {
			...
			public void getName(AccessibleEvent e) {
				...
				if (filterTextString.length() == 0 || filterTextString.equals(initialText)) {
					...
					e.result = filterTextString + ( " >" + getFilteredItemsCount() );
				}
			}
	...
	protected void textChanged() {
		narrowingDown = previousFilterText == null
				|| previousFilterText.equals("filter")
	...
	public void addSmartFilterListener(SelectionListener listener) {
		
		if (smartFilterCheckbox != null && !smartFilterCheckbox.isDisposed()) {
			smartFilterCheckbox.addSelectionListener(listener);
		}
