...
public class ArchiveInfoItemHandler {
    ...
    public void archivate(@Optional
    	...
    	if (infoItem != null) {
			...
			dialog.setMessage("You want really archive " + infoItem.getName() +" ?");
			...
			if (result == SWT.OK) {
				...
				try {
						matrosService.archivareItem(infoItem.getIdentifier());
						
						 // Notify the List to be refreshed
						 eventBroker.post(MyEventConstants.TOPIC__REFRESH_ITEM_ARCHIV, infoItem);

						 
						NotificationNote note = new NotificationNote();
						note.setHeading("Element achivated " + infoItem.getName());
